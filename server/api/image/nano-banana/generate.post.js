/**
 * Nano Banana 文生图接口（代理）
 * 后端：POST /api/image/nano-banana/generate
 * 请求体：model=nano-banana, prompt, outputFormat?, imageSize?
 */
export default defineEventHandler(async (event) => {
  const body = await readBody(event)
  const backendUrl = process.env.NODE_ENV === 'production'
    ? 'https://www.fuseaitools.com/api/image/nano-banana/generate'
    : 'http://127.0.0.1:8080/api/image/nano-banana/generate'
  try {
