/**
 * Runway Aleph 视频转换接口（代理）
 * 后端：POST /api/video/runway/aleph
 * 请求体：prompt, videoUrl, waterMark?, aspectRatio?, seed?, referenceImageUrl?
 */
export default defineEventHandler(async (event) => {
  const body = await readBody(event)
  const backendUrl = process.env.NODE_ENV === 'production'
    ? 'https://www.fuseaitools.com/api/video/runway/aleph'
    : 'http://127.0.0.1:8080/api/video/runway/aleph'
  try {
