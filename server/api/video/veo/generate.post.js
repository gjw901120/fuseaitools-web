/**
 * Veo 3 视频生成接口（代理）
 * 后端：POST /api/video/veo/generate
 * 请求体：prompt, imageUrls?, model?, generationType?, watermark?, aspectRatio?, seeds?, enableTranslation?
 */
export default defineEventHandler(async (event) => {
  const body = await readBody(event)
  const backendUrl = process.env.NODE_ENV === 'production'
    ? 'https://www.fuseaitools.com/api/video/veo/generate'
    : 'http://127.0.0.1:8080/api/video/veo/generate'
  try {
