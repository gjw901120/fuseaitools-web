...
## 21. Grok Imagine 对齐清单（Image + Video 双组件）

Grok 六子页 + `/home/grok` 聚合页。**Image 两工作流** → `GrokImageSeoContent`；**Video 四工作流** → `GrokVideoSeoContent`。聚合 intro/features/sections 同时覆盖 image 与 video。

### 21.1 涉及文件

- `pages/home/grok/*.vue`（6 子页）  
- `pages/home/grok.vue`  
- `components/tools/GrokImageSeoContent.vue`（T2I / I2I）  
- `components/tools/GrokVideoSeoContent.vue`（T2V / I2V / Upscale / Extend）  
- `components/tools/GrokToolCore.vue`（`imageDesc` / `videoDesc`）  
- `data/toolOverviews.js` → `grok`

### 21.2 API 参数（写作核对）

| 工作流 | modelKey | 关键约束 |
|------|----------|----------|
| Text to Image | `grok-imagine-text-to-image` | prompt 必填 ≤5000；当前表单固定 `aspect_ratio: 1:1` |
| Image to Image | `grok-imagine-image-to-image` | **恰好 1** 个 image URL；prompt 可选 ≤5000 |
| Text to Video | `grok-imagine-text-to-video` | prompt 必填；aspect 2:3–9:16；mode fun/normal/spicy；6s/10s；480p/720p |
| Image to Video | `grok-imagine-image-to-video` | 图片和/或 prompt；最多 7 图；spicy 有图时禁用 |
| Upscale | `grok-imagine-upscale` | Original Task ID；无 prompt |
| Extend | `grok-imagine-extend` | task ID + prompt；extend_at；extend 6s/10s；480p/720p |

计费：Image / Upscale → `getPrice(modelKey)`；T2V / I2V / Extend → `getPrice(modelKey, { duration, quality: resolution })`（见 `PRICING_MAPPING.md` § Grok）。

### 21.3 实施自检

**Image 子页（2）**

- [x] `GrokImageSeoContent` + `workflow*Map`  
- [x] `#below-main`、meta（20 free credits、`priceFromApi`）  
- [x] FAQ 4 条/页（2 专属 + 2 common，question 与 video 组件 **不重复**）  
- [x] 底部互链 → Grok Video  

**Video 子页（4）**

- [x] `GrokVideoSeoContent` + `workflow*Map`  
- [x] 四工作流对比表 + Technical specs  
- [x] FAQ 4 条/页；Upscale / Extend 专属 FAQ **不共用同一 question 文案**  
- [x] `watchRouteScroll` 仅匹配四 video path  
- [x] 底部互链 → Grok Image  

**FAQ 去重规则（Grok 实例）**

- Image `commonFaq` 问法含 **image**；Video `commonFaq` 问法含 **video**—跨组件不撞题。  
- 单页内：`workflowFaqMap` + `commonFaq` 合并后 question 唯一（各 4 条）。  
- 避免在 workflow FAQ 重复 common 已覆盖的 **定价/20 credits** 细节。  

**聚合页 `/home/grok`**

- [x] category `Image & Video`；features 6 项（Image · / Video · 前缀）  
- [x] sections 3 段：Image / Video / Try  

**组件**：`GrokImageSeoContent.vue` / `GrokVideoSeoContent.vue`
