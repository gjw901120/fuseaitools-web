...
## 21. Grok Imagine Image 对齐清单

Grok **图像**两子页 + `/home/grok` 聚合页（**暂不处理** video 四子页 SEO）。

### 21.1 涉及文件

- `pages/home/grok/text-to-image.vue`  
- `pages/home/grok/image-to-image.vue`  
- `pages/home/grok.vue`  
- `components/tools/GrokImageSeoContent.vue`（两页共用）  
- `components/tools/GrokToolCore.vue`（`imageDesc`）  
...
计费：ONCE/RULE，`getPrice(modelKey)`，无额外 formFields（见 `PRICING_MAPPING.md` § Grok）。

### 21.3 实施自检

**子页（2 工作流）**

- [x] 单组件 `GrokImageSeoContent` + `workflow*Map` 按 path 区分  
- [x] 每页独立 `useToolSEOAsync`（含 20 free credits、`priceFromApi`）  
- [x] FAQ ≥ 4 + `useToolSeoFaqSchema`  
- [x] T2I vs I2I 对比表 + Technical specs  
- [x] `#below-main` + 可滚动 `tool-page`  
- [x] `watchRouteScroll` 仅匹配两图像 path  

**聚合页 `/home/grok`**

- [x] intro / features / sections 仅图像两工作流（3 sections）  

**未做（刻意）**

- [ ] video 四页（T2V / I2V / Upscale / Extend）SEO 组件与 meta 增强  

**组件**：`GrokImageSeoContent.vue`

---

**文档版本**：1.7  
**最后更新**：2026-05-20  
**维护**：Wan §9、HappyHorse §10、Seedance §11、Seedream §12、Suno §13、ElevenLabs §14、Kling §15、**Flux Kontext §19**、**Ideogram §20**、**Grok Image §21** 为参考实现；**多版本拆分与向上互链**见 **§3.3**。路由/计费变更时同步对应 API 表、`PRICING_MAPPING.md`、`llms.txt`。
