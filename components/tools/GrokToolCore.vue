        ...
        <form class="config-form" @submit.prevent="generate">
          <fieldset class="config-fieldset" :disabled="loading">
            <div v-if="mode !== 'upscale'" class="form-group">
              ...
              <textarea v-model="form.prompt" class="form-input" rows="4" :maxlength="5000" :placeholder="placeholder"></textarea>
              <div v-if="form.prompt" class="char-count">{{ form.prompt.length }}/5000</div>
            </div>

            <div v-if="mode === 'upscale' || mode === 'extend'" class="form-group">
              <label class="form-label">Task ID <span class="required">*</span></label>
              <div class="select-with-arrow">
                <select v-model="form.taskId" class="form-input">
                  <option disabled value="">Select a task ID</option>
                  <option v-for="taskId in extendTaskIdOptions" :key="taskId" :value="taskId">{{ taskId }}</option>
                </select>
...
const extendTaskIdOptions = ['task_grok_12345678', 'task_grok_87654321']
...
async function handleFiles(files) {
  ...
  const res = await fetch(batchUploadUrl, { method: 'POST', body: fd, credentials: 'include' })
...
async function generate() {
  ...
  try {
    ...
    if (mode.value === 'text-to-image') {
      ...
      body = { model: 'grok-imagine-text-to-image', prompt: form.prompt.trim(), aspectRatio: form.aspectRatio }
      ...
      body = { model: 'grok-imagine-text-to-video', prompt: form.prompt.trim(), aspect_ratio: form.aspectRatio, mode: form.motionMode, duration: form.duration, resolution: form.resolution }
    } else if (mode.value === 'image-to-video') {
      api = '/api/video/grok/image-to-video'
      body = { model: 'grok-imagine-image-to-video', prompt: form.prompt.trim() || undefined, image_urls: form.imageUrls, aspect_ratio: form.aspectRatio, mode: form.motionMode, duration: form.duration, resolution: form.resolution }
    } else if (mode.value === 'upscale') {
      api = '/api/video/grok/upscale'
      body = { model: 'grok-imagine-upscale', task_id: form.taskId.trim() }
    } else {
      api = '/api/video/grok/extend'
      body = { model: 'grok-imagine-extend', task_id: form.taskId.trim(), prompt: form.prompt.trim(), extend_at: Number(form.extendAt) || 0, extend_times: form.duration }
    }
